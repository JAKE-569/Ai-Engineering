  ...
  return <div className="fixed inset-0 z-[120] bg-slate-950/80 p-3 md:p-6">
    <div className="mx-auto flex h-full max-w-[1500px] flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-[250px_minmax(0,1fr)_360px]">
        ...
        <aside className="min-h-0 overflow-y-auto border-l border-slate-200 bg-white p-5"><div className="border-b border-slate-200 pb-4"><div className="mb-2 flex gap-2"><span className="rounded-full bg-red-100 px-2 py-1 text-[11px] font-bold text-red-700">심각도: {activeMarkup?.severity || errorItem.severity}</span><span className="rounded-full bg-emerald-100 px-2 py-1 text-[11px] font-bold text-emerald-700">AI 검토</span></div><select value={activeMarkup?.id || ''} onChange={(e) => setSelectedMarkupId(e.target.value)} className="mb-3 w-full rounded-lg border border-slate-300 bg-white p-2 text-xs font-bold text-slate-800"><option value="">검토의견을 선택하세요</option>{markups.map((markup, index) => <option key={markup.id || index} value={markup.id}>{index + 1}. {markup.title} {markup.pageNumber ? `(Page ${markup.pageNumber})` : ''}</option>)}</select><h3 className="text-lg font-bold leading-snug text-slate-900">{activeMarkup?.title || errorItem.description}</h3><p className="mt-2 text-xs text-slate-500">{source?.name || errorItem.dwgFile}</p></div><section className="mt-5"><h4 className="mb-2 text-xs font-bold uppercase tracking-wide text-slate-500">검토 근거 (LOGIC)</h4><div className="rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm leading-6 text-slate-700">{activeMarkup?.codeClause || errorItem.suggestedFix || selectedReview?.description || '선택한 검토의견의 근거가 없습니다.'}</div></section><section className="mt-5"><h4 className="mb-2 text-xs font-bold uppercase tracking-wide text-slate-500">증거 데이터 (EVIDENCE)</h4><div className="grid grid-cols-2 gap-2"><div className="rounded-lg border border-red-200 bg-red-50 p-3"><span className="text-[10px] text-red-600">검토 상세</span><b className="mt-1 block text-sm text-red-800">{activeMarkup?.comment || errorItem.description}</b></div><div className="rounded-lg border border-slate-200 bg-slate-50 p-3"><span className="text-[10px] text-slate-500">페이지</span><b className="mt-1 block text-sm text-slate-800">{activeMarkup?.pageNumber || 1}</b></div></div></section><section className="mt-5 border-t border-slate-200 pt-4"><h4 className="mb-2 text-xs font-bold uppercase tracking-wide text-slate-500">전체 검토의견</h4><div className="space-y-2">{markups.map((markup, index) => <button key={markup.id || index} onClick={() => setSelectedMarkupId(markup.id)} className={`w-full rounded-lg border p-3 text-left text-xs ${activeMarkup?.id === markup.id ? 'border-blue-500 bg-blue-50' : 'border-slate-200 bg-white'}`}><b className="block text-slate-800">{index + 1}. {markup.title}</b><span className="mt-1 block text-slate-500">{markup.comment}</span></button>)}</div></section></aside>
      </div>
    </div>
  </div>;
};
