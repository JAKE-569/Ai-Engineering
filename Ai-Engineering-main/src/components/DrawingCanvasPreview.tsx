...
export const DrawingCanvasPreview: React.FC<DrawingCanvasPreviewProps> = ({
  ...
  const markupCardTone = (markup: ReviewMarkup) => {
    const category = `${markup.category || ''} ${markup.title || ''} ${markup.comment || ''}`;
    if (/원가|VE|절감|cost/i.test(category)) return 'border-emerald-500 bg-emerald-950/95';
    if (/안전|법규|safety/i.test(category)) return 'border-amber-500 bg-amber-950/95';
    return markup.severity === 'CRITICAL' ? 'border-red-500 bg-red-950/95' : markup.severity === 'WARNING' ? 'border-orange-500 bg-orange-950/95' : 'border-blue-500 bg-blue-950/95';
  };
      ...
      <div className={`relative flex h-full min-h-[420px] flex-col overflow-hidden rounded-lg bg-slate-100 ${className}`}>
        ...
        <div onClick={() => { setActiveMarkupId(null); onMarkupSelect?.(''); }} onWheel={handleCtrlWheelZoom} className="relative flex min-h-0 flex-1 items-center justify-center overflow-auto bg-slate-200 p-4">
          <div className="relative inline-block max-h-full max-w-full shadow-xl" style={{ transform: `translate(${pan.x}px, ${pan.y}px) scale(${zoomLevel})` }}>
            ...
            {activeId && (() => { const selected = effectiveMarkups.find((item) => item.id === activeId); if (!selected) return null; const left = Math.min(78, Math.max(22, selected.xPercent || 50)); const top = selected.yPercent || 50; const cardTop = top < 35 ? Math.min(72, top + 10) : Math.max(4, top - 27); return <><svg className="pointer-events-none absolute inset-0 z-10 h-full w-full overflow-visible"><line x1={`${selected.xPercent}%`} y1={`${top}%`} x2={`${left}%`} y2={`${cardTop}%`} stroke="currentColor" className={markupTone(selected).replace('bg-', 'text-')} strokeWidth="2" strokeDasharray="4 3" /></svg><div className={`absolute z-30 w-64 -translate-x-1/2 rounded-lg border-2 p-3 text-xs text-white shadow-2xl ${markupCardTone(selected)}`} style={{ left: `${left}%`, top: `${cardTop}%` }}><div className="font-bold text-white">{selected.title}</div><p className="mt-1 leading-5 text-white/90">{selected.comment}</p>{selected.codeClause && <span className="mt-2 inline-block rounded bg-white/20 px-2 py-1 text-[10px] font-bold text-white">{selected.codeClause}</span>}</div></>; })()}
          </div>
        </div>
