      ...
      <div className={`relative flex h-full min-h-[420px] flex-col overflow-hidden rounded-lg bg-slate-100 ${className}`}>
        ...
        <div onWheel={handleCtrlWheelZoom} className="relative flex min-h-0 flex-1 items-center justify-center overflow-auto bg-slate-200 p-4">
          <div className="relative inline-block max-h-full max-w-full shadow-xl" style={{ transform: `translate(${pan.x}px, ${pan.y}px) scale(${zoomLevel})` }}>
            ...
            {showMarkupLayer && effectiveMarkups.filter((mk) => !mk.pageNumber || mk.pageNumber === currentPage + 1).map((mk, index) => <button type="button" key={mk.id || index} onClick={() => setActiveMarkupId(mk.id)} className={`absolute z-20 h-7 w-7 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-white text-[10px] font-bold text-white shadow-lg ${markupTone(mk)}`} style={{ left: `${mk.xPercent}%`, top: `${mk.yPercent}%` }}>{index + 1}</button>)}
          </div>
        </div>
